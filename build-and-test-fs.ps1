# Manual build-and-test for the Phase 4 fs capability seam (Hsh.Fs + Hsh.Fs.Tests).
#
...
# tests\Hsh.Fs.Tests` work normally.
...
$fs = Get-ChildItem (Join-Path $src 'Hsh.Fs') -Filter '*.cs' | ForEach-Object FullName
$tests = Get-ChildItem (Join-Path $root 'tests\Hsh.Fs.Tests') -Filter '*.cs' | ForEach-Object FullName

# Compile in dependency order: Cordis.Core -> Hsh.Llm -> Hsh.Session -> Hsh.Tools -> Hsh.Fs -> tests.
...
$fsDll = Join-Path $bin 'Hsh.Fs.dll'
...
Invoke-Csc -ExtraArgs @('-target:library', "-out:$fsDll", "-r:$core", "-r:$llmDll", "-r:$sessionDll", "-r:$toolsDll") -Sources $fs -Label 'Hsh.Fs'
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$(Join-Path $bin 'Hsh.Fs.Tests.dll')", "-r:$core", "-r:$llmDll", "-r:$sessionDll", "-r:$toolsDll", "-r:$fsDll") -Sources $tests -Label 'Hsh.Fs.Tests'
...
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Hsh.Fs.Tests.runtimeconfig.json') -Encoding utf8

Write-Host '== Running Hsh.Fs.Tests =='
& dotnet (Join-Path $bin 'Hsh.Fs.Tests.dll')
exit $LASTEXITCODE
