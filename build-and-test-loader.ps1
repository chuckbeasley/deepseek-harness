# Manual build-and-test for the Phase 1 loader port worktree.
#
...
# tests\Cordis.Plugin.Loader.Tests` work normally.
...
$tests = Get-ChildItem (Join-Path $root 'tests\Cordis.Plugin.Loader.Tests') -Filter '*.cs' | ForEach-Object FullName

# Compile in dependency order: Cordis.Core first, then the Loader (-> Core), then the tests app.
$coreDll = Join-Path $bin 'Cordis.Core.dll'
$loaderDll = Join-Path $bin 'Cordis.Plugin.Loader.dll'
$testsDll = Join-Path $bin 'Cordis.Plugin.Loader.Tests.dll'
...
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$testsDll", "-r:$coreDll", "-r:$loaderDll") -Sources $tests -Label 'Cordis.Plugin.Loader.Tests'
...
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Cordis.Plugin.Loader.Tests.runtimeconfig.json') -Encoding utf8

Write-Host '== Running Cordis.Plugin.Loader.Tests =='
& dotnet $testsDll
exit $LASTEXITCODE
