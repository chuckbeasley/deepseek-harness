...
public static class ConnectionTests
{
    ...
    {
        var connection = new LspConnection(Spec(LspTestHarness.Env(("LSP_FAKE_ECHO_ENV", "HSH_LSP_TEST_FACT"), ("HSH_LSP_TEST_FACT", "managed"))), LspConnection.DefaultSpawner, NullHandler);
        try
        {
    ...
    public static async Task ScrubAmbientHshFacts_BeforeMergingExplicitEnv()
    {
        var old = Environment.GetEnvironmentVariable("HSH_LSP_SCRUB_ECHO");
        Environment.SetEnvironmentVariable("HSH_LSP_SCRUB_ECHO", "stale");
        try
        {
            var connection = new LspConnection(Spec(LspTestHarness.Env(("LSP_FAKE_ECHO_ENV", "HSH_LSP_SCRUB_ECHO"))), LspConnection.DefaultSpawner, NullHandler);
            try
            {
                Initialize(connection);
                var hover = await connection.Request("textDocument/hover", null).WaitAsync(TimeSpan.FromSeconds(30));
                Assert.True(hover.HasValue && hover.Value.TryGetProperty("contents", out var contents) && contents.GetString() == "<HSH_LSP_SCRUB_ECHO unset>", "the ambient HSH_ fact is scrubbed from the child");
            }
            finally
            {
                await CleanupAsync(connection);
        ...
        {
            Environment.SetEnvironmentVariable("HSH_LSP_SCRUB_ECHO", old);
        }
    }
