...
public sealed class Harness : IDisposable
{
    ...
    {
        var ctx = new Context();
        var root = Path.Combine(Path.GetTempPath(), "dsh-spill-tests-" + Guid.NewGuid().ToString("N"));
        var outside = Path.Combine(Path.GetTempPath(), "dsh-spill-outside-" + Guid.NewGuid().ToString("N"));
