...
public sealed class Fixture : IDisposable
{
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "hsh-identity-tests-" + Guid.NewGuid().ToString("N"));
...
public static class IdentityTests
{
    ...
    public static void HshHomeEnv_IsRespected()
    {
        using var fixture = Fixture.WithHome(out var home);
        var env = new Dictionary<string, string?> { [HomePaths.HshHomeEnv] = home };
        ...
        Assert.True(File.Exists(Path.Combine(home, AnonymousIdentityProvider.AnonymousUserIdFileName)),
            "$HSH_HOME must locate the id file");
    }
