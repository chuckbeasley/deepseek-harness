...
public static class LaunchTests
{
    ...
    {
        var runtime = SdkLaunch.ResolveLaunch(new HarnessClientOptions { HshBin = "C:\\hsh\\Harness.Cli.dll" }, "C:\\work");
        Assert.Equal("dotnet", runtime.Command, "a .dll entry spawns via dotnet");
        Assert.True(runtime.Args.SequenceEqual(new[] { "C:\\hsh\\Harness.Cli.dll", "--profile", "sdk" }), "the canonical argv");
        Assert.Equal("hsh profile \"sdk\"", runtime.Description, "the description names the default profile");
    ...
    public static void ResolveLaunch_AppliesTheOverrides()
    {
        ...
        Assert.Equal("C:\\hsh\\hsh.exe", runtime.Command, "an apphost entry spawns directly");
        ...
        Assert.Equal("C:\\caller\\home", runtime.Environment()["HSH_HOME"], "the home resolves and lands in the child env");
