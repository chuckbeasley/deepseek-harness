...
/// <summary>One disposable temp directory used as the child's HSH_HOME and cwd.</summary>
internal sealed class TempDir : IDisposable
{
    private TempDir()
    {
        Path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "hsh-sdk-client-" + Guid.NewGuid().ToString("N"));
...
/// <summary>The built hsh CLI entry and per-test runtime resolution.</summary>
internal static class Runtime
{
    ...
    public static RuntimeProcessOptions Resolve(string hshHome, string cwd)
        => SdkLaunch.ResolveLaunch(new HarnessClientOptions { HshBin = CliPath, HshHome = hshHome, ProcessCwd = cwd },
    ...
    private static string LoadCliPath()
    {
        var metadata = typeof(Runtime).Assembly.GetCustomAttributes(typeof(System.Reflection.AssemblyMetadataAttribute), inherit: false)
            ...
            .FirstOrDefault(attribute => attribute.Key == "HshCliPath");
        var path = metadata?.Value;
        if (string.IsNullOrEmpty(path) || !File.Exists(path))
        {
            throw new InvalidOperationException($"the test build did not locate the hsh CLI at \"{path}\"");
        }
        return path!;
    }
}
