...
/// <c>snapshots/session/*</c> scenario replays through the real hsh CLI and its persisted log,
...
public static class CorpusTests
{
    ...
    private static ScenarioOutcome RunScenario(ScenarioManifest scenario, IReadOnlyDictionary<string, string> compositionOwners)
    {
        ...
        try
        {
            ...
            if (scenario.Permission is not null) env["HSH_PERMISSION_MODE"] = scenario.Permission;
            ApplyScenarioEnv(env, scenario, model.Model, scenariosDir, compositionOwners);
            var childFiles = ChildFilesEnv(dir);
            if (childFiles is not null) env["HSH_SNAPSHOT_CHILD_FILES"] = childFiles;
    ...
    public static void DiffScenario(string name, Action<string> report)
    {
        ...
        try
        {
            ...
            if (manifest.Permission is not null) env["HSH_PERMISSION_MODE"] = manifest.Permission;
    ...
    public static string StderrFromSession(string log)
    {
        ...
        return output.ToString() + $"hsh: {code}: {message}\n";
    }
        ...
        string scenariosDir, IReadOnlyDictionary<string, string> compositionOwners)
    {
        ...
        if (metaJson is not null) env["HSH_SNAPSHOT_MODEL_META"] = metaJson;
        var diffBasis = DiffBasisEnv(scenariosDir, scenario.Name, scenario.Composition, compositionOwners);
        if (diffBasis is not null) env["HSH_FS_DIFF_BASIS_MAX_BYTES"] = diffBasis;
        var spillCap = SpillCapEnv(scenariosDir, scenario.Name, scenario.Composition, compositionOwners);
        if (spillCap is not null) env["HSH_SNAPSHOT_SPILL_MAX_BYTES"] = spillCap;
        env["HSH_SNAPSHOT_SPILL_ROOT"] = SpillRoot(scenario.Name);
        var compactionMaxTokens = CompactionMaxTokensEnv(scenariosDir, scenario.Name, scenario.Composition, compositionOwners);
        if (compactionMaxTokens is not null) env["HSH_SNAPSHOT_COMPACTION_MAX_TOKENS"] = compactionMaxTokens;
        var lspConfig = LspConfigEnv(scenariosDir, scenario.Name, scenario.Composition, compositionOwners);
        if (lspConfig is not null) env["HSH_SNAPSHOT_LSP_CONFIG"] = lspConfig;
    ...
    private static IReadOnlyDictionary<string, string>? SubagentSnapshotEnv(string scenariosDir, string scenarioName, string composition, IReadOnlyDictionary<string, string> compositionOwners)
    {
        ...
        {
            env["HSH_SNAPSHOT_SUBAGENT_DIAGNOSTIC"] = "1";
        }
        if (text.Contains("subagent-acp-diagnostic", StringComparison.Ordinal))
        {
            env["HSH_SNAPSHOT_SUBAGENT_ACP"] = "1";
        }
    ...
    /// HSH_HOOKS_CC_CONFIG / HSH_HOOKS_CODEX_CONFIG (the scenario's own cordis patches mount the
    ...
    private static IReadOnlyDictionary<string, string>? HookConfigEnv(string dir)
    {
        ...
        {
            [dialect == "codex" ? "HSH_HOOKS_CODEX_CONFIG" : "HSH_HOOKS_CC_CONFIG"] = Path.GetFullPath(path),
        };
    }
    ...
    /// <summary>The deterministic snapshot spill root (mirror of the TS snapshotSpillRoot): /tmp/hsh-acp-snap-&lt;sha256-9&gt;.</summary>
    private static string SpillRoot(string scenarioName)
    {
        ...
        return $"/tmp/hsh-acp-snap-{hash.Substring(0, 9)}";
    }
    ...
    /// tool-lsp location cap, as the HSH_SNAPSHOT_LSP_CONFIG env JSON; null when the scenario
    ...
    /// <summary>The web snapshot channels: HSH_SNAPSHOT_WEB_FETCH when the scenario mounts the
    ...
    private static IReadOnlyDictionary<string, string>? WebSnapshotEnv(string scenariosDir, string scenarioName, string composition, IReadOnlyDictionary<string, string> compositionOwners)
    {
        ...
        if (fetchFixture) env["HSH_SNAPSHOT_WEB_FETCH"] = "1";
        ...
        foreach (var line in lines)
        {
            ...
            if (baseUrl.Success)
            {
                env["HSH_SNAPSHOT_WEB_SEARCH_BASE_URL"] = baseUrl.Groups[1].Value.Trim('\'', '"');
                continue;
            }
            var apiKey = Regex.Match(line, @"^\s+apiKey:\s*(\S+)\s*$");
            if (apiKey.Success)
            {
                env["HSH_SNAPSHOT_WEB_SEARCH_API_KEY"] = apiKey.Groups[1].Value.Trim('\'', '"');
                continue;
            }
        }
        if (env.ContainsKey("HSH_SNAPSHOT_WEB_SEARCH_BASE_URL"))
        {
            // The recorded error fixture answers the recorded authority.
            env["HSH_SNAPSHOT_WEB_SEARCH_FIXTURE"] = "1";
        }
        return env.Count > 0 ? env : null;
    }
    ...
    /// recorded model, as the HSH_SNAPSHOT_MODEL_META env JSON; null when the scenario declares
    /// none.
    /// </summary>
    ...
    private static void PrepareWorkspace(string cwd, string? setup)
    {
        switch (setup)
        {
            ...
            case "editing-cordis-skill":
                var skillDir = Path.Combine(cwd, ".hsh", "skills", "editing-cordis-compositions");
                ...
                var realSkill = Path.Combine(
                    SnapshotDriver.RepoRoot(), "tests", "Hsh.Snapshot.Tests", "fixtures",
