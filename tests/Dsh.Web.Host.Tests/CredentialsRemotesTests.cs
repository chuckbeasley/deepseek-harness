...
public static class CredentialsRemotesTests
{
    ...
    private static (Context Ctx, DshRpcRegistry Registry, LocalCredentialsProvider Credentials) Boot(
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "dsh-host-creds-" + Guid.NewGuid().ToString("N"));
