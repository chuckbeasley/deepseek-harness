...
public static class SettingsOpenersTests
{
    ...
    public static void CanOpenAgentPresetDirectory_AnswersTheDeploymentFact()
    {
        ...
        try
        {
            var registry = new HshRpcRegistry(withoutCtx);
        ...
        try
        {
            var registry = new HshRpcRegistry(defaultCtx);
    ...
    private static (Context Ctx, HshRpcRegistry Registry) Boot(string documentPath)
    {
        var ctx = new Context();
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "hsh-host-openers-" + Guid.NewGuid().ToString("N"));
