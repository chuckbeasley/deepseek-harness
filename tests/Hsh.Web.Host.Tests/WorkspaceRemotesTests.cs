...
public static class WorkspaceRemotesTests
{
    ...
    private static RpcResponse Invoke(HshRpcRegistry registry, string endpoint, object args)
        => registry.InvokeAsync(new RpcRequest(endpoint, JsonSerializer.SerializeToElement(args))).GetAwaiter().GetResult();

    private static (Context Ctx, HshRpcRegistry Registry, WorkspaceRegistry Seam, string Root) Boot()
    {
        var ctx = new Context();
        var root = Path.Combine(Path.GetTempPath(), "hsh-host-workspace-" + Guid.NewGuid().ToString("N"));
