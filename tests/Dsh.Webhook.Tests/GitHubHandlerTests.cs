...
public static class GitHubHandlerTests
{
    ...
    private sealed class TempHome : IDisposable
    {
        public TempHome()
        {
            Path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "dsh-webhook-tests-" + Guid.NewGuid().ToString("N"));
    ...
    private sealed class Spine : IDisposable
    {
        ...
        {
            var ctx = new Context();
            var credentials = new LocalCredentialsProvider(ctx, new LocalCredentialsConfig(DshHome: System.IO.Path.Combine(System.IO.Path.GetTempPath(), "dsh-webhook-creds-" + Guid.NewGuid().ToString("N"))));
