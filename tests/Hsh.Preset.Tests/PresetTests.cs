...
internal static class PresetFixture
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "hsh-preset-tests-" + Guid.NewGuid().ToString("N"));
...
public static class PresetTests
{
    public static void DiscoveryListsPresetsFromATempRoot()
    {
        ...
        try
        {
            PresetFixture.Write(Path.Combine(root, "standard", "agent.cordis.yml"), "- name: '@deepseek-ai/hsh-tool-fs'\n");
            PresetFixture.Write(Path.Combine(root, "minimal", "agent.cordis.yml"), "- name: '@deepseek-ai/hsh-persona'\n");
    ...
    public static void ResolveComposesLayersThroughPatches()
    {
        ...
        try
        {
            PresetFixture.Write(Path.Combine(root, "standard", "agent.cordis.yml"),
                "- id: persona\n" +
                "  name: '@deepseek-ai/hsh-persona'\n" +
                "  config:\n" +
                "    text: You are the preset identity.\n" +
                "- id: tool-bash\n" +
                "  name: '@deepseek-ai/hsh-tool-bash'\n");
            var patches = new List<object?>
            {
                ...
                new Dictionary<string, object?>
                {
                    ...
                    {
                        new Dictionary<string, object?> { ["id"] = "tool-extra", ["name"] = "@deepseek-ai/hsh-tool-extra" },
                    },
                },
            };
            ...
            Assert.Equal("@deepseek-ai/hsh-tool-extra", composed.Rows[2].Name, "the inserted row must carry its plugin name");
        }
        finally
        {
            PresetFixture.Remove(root);
    ...
    public static void ResolveComposesGroupRows()
    {
        ...
        try
        {
            PresetFixture.Write(Path.Combine(root, "grouped", "agent.cordis.yml"),
                ...
                "  config:\n" +
                "    - id: plan-mode\n" +
                "      name: '@deepseek-ai/hsh-plan-mode'\n");
            ...
            Assert.Equal("@deepseek-ai/hsh-plan-mode", children[0].Name, "the child row must carry its plugin name");
        }
        finally
        {
            PresetFixture.Remove(root);
