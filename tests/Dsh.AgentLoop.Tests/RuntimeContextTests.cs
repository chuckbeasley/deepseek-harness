...
public static class RuntimeContextTests
{
    public static async Task RunAsync(Harness h)
    {
        var provider = new Func<Task<RuntimeContextPart>>(
            () => Task.FromResult(new RuntimeContextPart("Current working directory: C:\\work", new[] { "cwd" })));
        ...
        Assert.Sequence(new[] { "cwd" }, source.Sections ?? Array.Empty<string>(), "the snapshot must attribute its contributing sections");
