...
public static class ArgsTests
{
    public static void BareInvocation_RequiresAProfile()
    {
        ...
        var result = Args.ParseHshArgs(Array.Empty<string>(), "9.9.9");
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "-h" }, "9.9.9");
        Assert.Equal(0, result.ExitCode, "help exits 0");
        Assert.True(console.Out.ToString().Contains("Usage: hsh"), "help prints the launcher usage");
    }

    public static void Version_PrintsAndExitsZero()
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "-V" }, "9.9.9");
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "--profile", "tui", "--resume", "abc" }, "9.9.9");
        var invocation = AssertInvocation<HshInvocation.ProfileInvocation>(result);
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "--profile", "web", "-h" }, "9.9.9");
        var invocation = AssertInvocation<HshInvocation.ProfileInvocation>(result);
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "--patch", "a.yml", "--patch", "b.yml", "--profile", "headless" }, "9.9.9");
        var invocation = AssertInvocation<HshInvocation.ProfileInvocation>(result);
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "web", "extra" }, "9.9.9");
        var invocation = AssertInvocation<HshInvocation.ProfileInvocation>(result);
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "--profile", "headless", "--dump-config", "--dump-default-config" }, "9.9.9");
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "--profile", "headless", "--dump-config", "extra" }, "9.9.9");
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "--profile", "headless", "--dump-default-config", "--patch", "x.yml" }, "9.9.9");
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "--profile", "tui", "plugin", "--profile", "tui", "add", "pkg" }, "9.9.9");
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "plugin", "add", "pkg" }, "9.9.9");
        ...
        result = Args.ParseHshArgs(new[] { "plugin", "--profile", "tui" }, "9.9.9");
    ...
    {
        using var console = new ConsoleCapture();
        var result = Args.ParseHshArgs(new[] { "plugin", "--profile", "tui", "add", "pkg" }, "9.9.9");
        var invocation = AssertInvocation<HshInvocation.PluginInvocation>(result);
        Assert.Equal("tui", invocation.Profile, "the plugin profile");
        Assert.Sequence(new[] { "add", "pkg" }, invocation.Args, "the plugin arguments");
    }

    private static T AssertInvocation<T>(ParseResult result) where T : HshInvocation
    {
