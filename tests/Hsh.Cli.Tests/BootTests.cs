...
public static class BootTests
{
    ...
    {
        using var home = new TempHshHome();
        using var console = new ConsoleCapture();
        var feed = Path.Combine(Path.GetTempPath(), "hsh-cli-feed-" + Guid.NewGuid().ToString("N"));
        try
        {
            ...
            Assert.True(console.Out.ToString().Contains("hsh: added harness.testbundle to profile headless"), "the add reports the profile");

            var profileDir = Path.Combine(home.Dir, "profiles", "headless");
            var manifest = ProfileBoot.ReadProfileManifest(profileDir);
            Assert.True(manifest.Hsh?.Profile?.Bundles?.Contains("harness.testbundle") == true, "the manifest records the installed bundle");
    ...
    private static void WriteBundlePackage(string feed, string id, string version, bool bundleContent)
    {
        ...
        using (var zip = System.IO.Compression.ZipFile.Open(path, System.IO.Compression.ZipArchiveMode.Create))
        {
            ...
            if (bundleContent)
            {
                ...
                {
                    writer.Write("{\n  \"name\": \"harness.testbundle\",\n  \"hsh\": { \"bundle\": { \"patch\": \"cordis.patch.yml\" } }\n}\n");
                }
            }
        }
    }
    ...
    public static void Plugin_InitializesAndManagesBundles()
    {
        ...
        Assert.Sequence(new[] { "@deepseek-ai/hsh-base", "@deepseek-ai/hsh-headless" }, listed, "the template bundles list");
        ...
        Assert.Sequence(new[] { "@deepseek-ai/hsh-base", "@deepseek-ai/hsh-headless", "my-bundle" }, listed, "the added bundle joins the list");
        ...
        listed = console.Out.ToString().Split('\n', StringSplitOptions.RemoveEmptyEntries).Select(line => line.TrimEnd('\r')).ToArray();
        Assert.Sequence(new[] { "@deepseek-ai/hsh-base", "@deepseek-ai/hsh-headless" }, listed, "the removed bundle leaves the list");
    }
