...
/// <summary>
/// One temp harness home for one test: HSH_HOME points at it and DEEPSEEK_API_KEY is cleared so
/// headless runs stay on the mock route. Both are restored on dispose.
/// </summary>
public sealed class TempHshHome : IDisposable
{
    ...
    public TempHshHome()
    {
        Dir = Path.Combine(Path.GetTempPath(), "hsh-cli-tests-" + Guid.NewGuid().ToString("N"));
        _oldHome = Environment.GetEnvironmentVariable("HSH_HOME");
        _oldKey = Environment.GetEnvironmentVariable("DEEPSEEK_API_KEY");
        Directory.CreateDirectory(Dir);
        Environment.SetEnvironmentVariable("HSH_HOME", Dir);
    ...
    public void Dispose()
    {
        Environment.SetEnvironmentVariable("HSH_HOME", _oldHome);
