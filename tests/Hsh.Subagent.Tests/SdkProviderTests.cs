...
public static class SdkProviderTests
{
    private sealed class TempHome : IDisposable
    {
        public TempHome()
        {
            Path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "hsh-subagent-tests-" + Guid.NewGuid().ToString("N"));
    ...
    private static SdkOutOfProcessProvider Provider(TempHome home, Dictionary<string, string> env, string? cwd = null)
        => new(new SdkOutOfProcessConfig(
            HshBin: typeof(SdkProviderTests).Assembly.Location,
            Profile: "sdk",
    ...
    public static void RunsATurn_EndToEnd_AndMintsDistinctIds()
    {
        ...
        var run1 = registry.StartAsync("hsh-sdk", new SubagentRequest("task one", "t1")).GetAwaiter().GetResult();
        var run2 = registry.StartAsync("hsh-sdk", new SubagentRequest("task two", "t2")).GetAwaiter().GetResult();
    ...
    public static void ChildRunsInTheConfiguredCwd_AndSeesExplicitEnv()
    {
        ...
        var provider = Provider(home, Script(
            ("FAKE_ECHO_CWD", "1"),
            ("FAKE_ECHO_ENV", "DEEPSEEK_API_KEY,HSH_HOME"),
            ("DEEPSEEK_API_KEY", "sk-explicit")), cwd: childCwd);
        using var registry = new InProcessSubagentProvider(new global::Harness.Cordis.Core.Context());
        using var registration = registry.RegisterProvider(provider);
        var run = registry.StartAsync("hsh-sdk", new SubagentRequest("task")).GetAwaiter().GetResult();
        ...
        Assert.True(result.Text.Contains($"HSH_HOME={home.Path}", StringComparison.Ordinal), "the harness home reached the child");
        run.DisposeAsync().GetAwaiter().GetResult();
    }

    public static void AmbientScrub_DropsHshAndSecretNames()
    {
        ...
        Assert.False(scrubbed.ContainsKey("HSH_HOME"), "HSH_* names are dropped");
    ...
    public static void CancelMidTurn_SettlesAbortedAndReapsTheChild()
    {
        ...
        using var registration = registry.RegisterProvider(provider);
        using var cts = new CancellationTokenSource();
        var run = registry.StartAsync("hsh-sdk", new SubagentRequest("task"), cts.Token).GetAwaiter().GetResult();
    ...
    public static void Registry_RejectsDuplicateAndUnknownProviders()
    {
        ...
        Assert.NotNull(registry.GetProvider("hsh-sdk"));
        Assert.True(registry.List().Any(candidate => candidate.Name == "hsh-sdk"));
    }
