...
public class FiberEffectTests
{
    [Fact]
    public async Task DisposeAsync_UnwindsEffectsInReverseRegistrationOrder()
    {
        var ctx = new Context();
    ...
    [Fact]
    public async Task FailingCleanup_IsContainedAndDoesNotStarvePeers()
    {
        var ctx = new Context();
    ...
    [Fact]
    public void EffectDisposer_IsSingleShot()
    {
    ...
    [Fact]
    public async Task Effect_DisposedBeforeUnload_IsNotRunAgainAtContextDispose()
    {
        var ctx = new Context();
        var count = 0;
        var disposer = ctx.Effect(() => new Disposer(() => count++));
    ...
    [Fact]
    public async Task Effect_OnDisposedContext_ThrowsInactiveEffect()
    {
        var ctx = new Context();
        await ctx.DisposeAsync();
    ...
    [Fact]
    public async Task NestedEffect_UnwindsBeforeItsParent()
    {
        var ctx = new Context();
        var order = new List<string>();
        ctx.Effect(() =>
    ...
    [Fact]
    public async Task EffectAsync_AwaitsAsyncCleanup()
    {
        var ctx = new Context();
        var finished = false;
    ...
    [Fact]
    public async Task ContextDispose_UnwindsListenerRegistrations()
    {
        var ctx = new Context();
        var count = 0;
