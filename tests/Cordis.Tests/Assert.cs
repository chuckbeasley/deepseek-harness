...
/// <summary>Minimal zero-dependency assertion helpers for the Phase 0 console test runner.</summary>
public static class Assert
{
    ...
    /// <summary>Asserts <paramref name="actual"/> deep-equals <paramref name="expected"/> (deep equality for collections).</summary>
    public static void Equal(object? expected, object? actual, string? message = null)
    {
        if (!ValuesEqual(expected, actual)) Fail(message ?? $"expected {expected} but got {actual}");
    }
    ...
    {
        if (ValuesEqual(expected, actual)) Fail(message ?? $"expected {expected} to differ from {actual}");
    }
    ...
    /// <summary>Asserts <paramref name="path"/> ends with <paramref name="suffix"/>.</summary>
    public static void EndsWith(string suffix, string path, string? message = null)
    {
        if (!path.EndsWith(suffix, StringComparison.Ordinal))
        {
            Fail(message ?? $"expected \"{path}\" to end with \"{suffix}\"");
        }
    }
    ...
    private static bool ValuesEqual(object? expected, object? actual)
        => Deep.DeepEqual(expected, actual, strict: true);
}
