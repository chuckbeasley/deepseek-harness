...
public static class SubprocessTests
{
    ...
    {
        var old = Environment.GetEnvironmentVariable("HSH_SCRUB_TEST");
        Environment.SetEnvironmentVariable("HSH_SCRUB_TEST", "stale");
        try
        {
            var service = Boot(out var ctx);
            using var _ = ctx;
            var handle = service.Spawn(new SubprocessSpawnSpec(
                new[] { "cmd.exe", "/d", "/s", "/c", "echo [%HSH_SCRUB_TEST%][%FOO%]" },
                Environment.CurrentDirectory,
            ...
            var outcome = await handle.Done;
            Assert.Equal(0, outcome.ExitCode, "the child exits 0");
            // cmd renders an undefined variable literally, so the literal %HSH_SCRUB_TEST% IS the proof of the scrub.
Assert.Equal("[%HSH_SCRUB_TEST%][bar]", ReadAll(handle.Collected.Stdout).TrimEnd(), "the managed HSH_ fact is scrubbed and the explicit entry merges");
        }
        finally
        {
            Environment.SetEnvironmentVariable("HSH_SCRUB_TEST", old);
        }
    }
