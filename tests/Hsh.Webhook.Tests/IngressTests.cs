...
public static class IngressTests
{
    ...
    private sealed class Spine : IDisposable
    {
        ...
        {
            var ctx = new Context();
            var credentials = new LocalCredentialsProvider(ctx, new LocalCredentialsConfig(HshHome: System.IO.Path.Combine(System.IO.Path.GetTempPath(), "hsh-webhook-ingress-" + Guid.NewGuid().ToString("N"))));
