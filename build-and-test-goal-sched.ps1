...
# unrestricted host, `dotnet build` / `dotnet run --project tests\Hsh.Goal.Tests` (and the other
...
$goalDll = Join-Path $bin 'Hsh.Goal.dll'
$scheduleDll = Join-Path $bin 'Hsh.Schedule.dll'
$feedbackDll = Join-Path $bin 'Hsh.Feedback.dll'
...
$goal = Get-ChildItem (Join-Path $src 'Hsh\Hsh.Goal') -Filter '*.cs' | ForEach-Object FullName
$schedule = Get-ChildItem (Join-Path $src 'Hsh\Hsh.Schedule') -Filter '*.cs' | ForEach-Object FullName
$feedback = Get-ChildItem (Join-Path $src 'Hsh\Hsh.Feedback') -Filter '*.cs' | ForEach-Object FullName
$goalTests = Get-ChildItem (Join-Path $root 'tests\Hsh.Goal.Tests') -Filter '*.cs' | ForEach-Object FullName
$scheduleTests = Get-ChildItem (Join-Path $root 'tests\Hsh.Schedule.Tests') -Filter '*.cs' | ForEach-Object FullName
$feedbackTests = Get-ChildItem (Join-Path $root 'tests\Hsh.Feedback.Tests') -Filter '*.cs' | ForEach-Object FullName

# Compile in dependency order: Cordis.Core -> Hsh.Llm -> Hsh.Session -> Hsh.Tools ->
# Cordis.Plugin.Timer -> Hsh.Goal -> Hsh.Schedule -> Hsh.Feedback, then the test executables.
...
Invoke-Csc -ExtraArgs @('-target:library', "-out:$goalDll", "-r:$core", "-r:$llmDll", "-r:$sessionDll", "-r:$toolsDll") -Sources $goal -Label 'Hsh.Goal'
Invoke-Csc -ExtraArgs @('-target:library', "-out:$scheduleDll", "-r:$core", "-r:$timerDll") -Sources $schedule -Label 'Hsh.Schedule'
Invoke-Csc -ExtraArgs @('-target:library', "-out:$feedbackDll", "-r:$core", "-r:$llmDll", "-r:$sessionDll", "-r:$toolsDll") -Sources $feedback -Label 'Hsh.Feedback'
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$(Join-Path $bin 'Hsh.Goal.Tests.dll')", "-r:$core", "-r:$llmDll", "-r:$sessionDll", "-r:$toolsDll", "-r:$goalDll") -Sources $goalTests -Label 'Hsh.Goal.Tests'
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$(Join-Path $bin 'Hsh.Schedule.Tests.dll')", "-r:$core", "-r:$timerDll", "-r:$scheduleDll") -Sources $scheduleTests -Label 'Hsh.Schedule.Tests'
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$(Join-Path $bin 'Hsh.Feedback.Tests.dll')", "-r:$core", "-r:$llmDll", "-r:$sessionDll", "-r:$toolsDll", "-r:$feedbackDll") -Sources $feedbackTests -Label 'Hsh.Feedback.Tests'
...
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Hsh.Goal.Tests.runtimeconfig.json') -Encoding utf8
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Hsh.Schedule.Tests.runtimeconfig.json') -Encoding utf8
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Hsh.Feedback.Tests.runtimeconfig.json') -Encoding utf8

Write-Host '== Running Hsh.Goal.Tests =='
& dotnet (Join-Path $bin 'Hsh.Goal.Tests.dll')
if ($LASTEXITCODE -ne 0) { throw "Hsh.Goal.Tests failed (exit $LASTEXITCODE)" }

Write-Host '== Running Hsh.Schedule.Tests =='
& dotnet (Join-Path $bin 'Hsh.Schedule.Tests.dll')
if ($LASTEXITCODE -ne 0) { throw "Hsh.Schedule.Tests failed (exit $LASTEXITCODE)" }

Write-Host '== Running Hsh.Feedback.Tests =='
& dotnet (Join-Path $bin 'Hsh.Feedback.Tests.dll')
if ($LASTEXITCODE -ne 0) { throw "Hsh.Feedback.Tests failed (exit $LASTEXITCODE)" }
