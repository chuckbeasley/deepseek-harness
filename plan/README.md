...
- Still deferred (named in the port spec): the rest of the remote catalog (session follow/control streams beyond page/follow, settings/mutate path ops + document/preset openers, workspace rename/delete/reorder/archive/follow — the durable registry, directoryPicker), the Roslyn source generator for typed codecs, the ui-* plugin set, locale dictionaries, and the trust/auth fence beyond loopback.
