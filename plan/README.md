...
- Still deferred (named in the port spec): the rest of the remote catalog (session control stream + queue/jobs/projection deltas, settings document/preset openers, workspace rename/delete/reorder/archive/follow — the durable registry), the `$events` waterfall/`$events/result` settlement (no waterfall events exist in the port yet), the Roslyn source generator for typed codecs, the ui-* plugin set, locale dictionaries, and auth beyond the loopback fence (trustedHosts authorities, persistent signing secret).
