...
- `src/Dsh/Dsh.Terminal` — the ConPTY backend (Windows): pseudo-console spawn sequence, controlled-prompt readiness (`stdin_read`/`inferred_idle`), OSC 133 sanitizer, serialized resize, ClosePseudoConsole → tree-kill teardown; suites self-skip on non-Windows.
- All wave-3 seams are mounted in the CLI spine (`dsh-base` bundle rows + `SpineRegistry`); the sandbox row selects its backend by config, and the `dotnet-ci.yml` lane now runs every console suite.
