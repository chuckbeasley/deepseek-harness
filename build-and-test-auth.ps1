...
# tests\Hsh.Authorization.Tests` work normally.
...
$authorization = Get-ChildItem (Join-Path $root 'src\Hsh\Hsh.Authorization') -Filter '*.cs' | ForEach-Object FullName
$tests = Get-ChildItem (Join-Path $root 'tests\Hsh.Authorization.Tests') -Filter '*.cs' | ForEach-Object FullName

# Compile in dependency order: Cordis.Core -> Hsh.Llm -> Hsh.Session -> Hsh.Tools ->
# Hsh.Credentials -> Hsh.Authorization -> the test app.
...
$authorizationDll = Join-Path $bin 'Hsh.Authorization.dll'
$testsDll = Join-Path $bin 'Hsh.Authorization.Tests.dll'
...
Invoke-Csc -ExtraArgs @('-target:library', "-out:$authorizationDll", "-r:$coreDll", "-r:$credentialsDll") -Sources $authorization -Label 'Hsh.Authorization'
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$testsDll", "-r:$coreDll", "-r:$credentialsDll", "-r:$authorizationDll") -Sources $tests -Label 'Hsh.Authorization.Tests'
...
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Hsh.Authorization.Tests.runtimeconfig.json') -Encoding utf8

Write-Host ''
Write-Host '== Running Hsh.Authorization.Tests =='
& dotnet $testsDll
exit $LASTEXITCODE
