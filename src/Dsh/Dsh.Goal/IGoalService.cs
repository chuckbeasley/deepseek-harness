...
public interface IGoalService
{
    ...
    GoalView? Get(Dsh.Session.Session session);
    ...
    GoalView? Disarm(Dsh.Session.Session session);
    ...
    GoalView Create(Dsh.Session.Session session, string objective, int? maxGoalRounds = null);
    ...
    GoalView Edit(Dsh.Session.Session session, GoalRef reference, string? objective, int? maxGoalRounds);
    ...
    GoalView Pause(Dsh.Session.Session session, GoalRef reference);
    ...
    GoalView Resume(Dsh.Session.Session session, GoalRef reference);

    /// <summary>Mark a current non-complete goal complete and disarm it.</summary>
    /// <param name="session">the owning session.</param>
    /// <param name="reference">the expected current revision.</param>
    /// <returns>the completed view.</returns>
    GoalView Complete(Dsh.Session.Session session, GoalRef reference);
    ...
    GoalView Block(Dsh.Session.Session session, GoalRef reference, GoalBlockReason reason);
    ...
    GoalRef Clear(Dsh.Session.Session session, GoalRef reference);
}
