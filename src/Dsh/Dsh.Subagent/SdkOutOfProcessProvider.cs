...
public sealed record SdkOutOfProcessConfig(
    ...
    /// <summary>Absolute child harness home (<c>DSH_HOME</c>).</summary>
    string DshHome,
...
/// <summary>
/// The DSH SDK out-of-process subagent driver (provider name <c>dsh-sdk</c>; port of
/// <c>@deepseek-ai/dsh-subagent-dsh-sdk</c>): spawns a complete child runtime per delegation,
...
public sealed class SdkOutOfProcessProvider : ISubagentProvider
{
    ...
    public SdkOutOfProcessProvider(SdkOutOfProcessConfig config)
    {
        ...
        if (!Path.IsPathFullyQualified(config.DshHome))
        {
            throw new ArgumentException($"subagent dshHome must be absolute, got \"{config.DshHome}\"", nameof(config));
        }
        if (!File.Exists(config.DshBin))
        {
            throw new ArgumentException($"subagent dshBin \"{config.DshBin}\" does not exist", nameof(config));
        }
...
{
    public const string ProviderLabel = "DSH SDK";
...
internal sealed class SdkChildConnection
{
    ...
    public static SdkChildConnection Spawn(SdkOutOfProcessConfig config, string cwd)
    {
        ...
        info.Environment["DSH_HOME"] = config.DshHome;
        if (config.DshBin.EndsWith(".dll", StringComparison.OrdinalIgnoreCase))
        {
            info.FileName = "dotnet";
            info.ArgumentList.Add(config.DshBin);
        }
        else
        {
            info.FileName = config.DshBin;
        }
