...
/// <c>dsh-session:</c> URIs carry a base64url-encoded JSON string id; mentions render as
...
public static class SessionReferenceUri
{
    ...
    public const string Scheme = "dsh-session:";
...
/// <summary>
/// Mention extraction (port of parseSessionReferenceText): Markdown <c>@[label](dsh-session:...)</c>
/// mentions and bare canonical <c>dsh-session:</c> URIs, in appearance order.
/// </summary>
...
/// <c>dsh-session:</c> mentions in the session's user messages are resolved through the injected
