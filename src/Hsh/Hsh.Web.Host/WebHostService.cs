...
public sealed class WebHostService : Service
{
    ...
    public override async ValueTask StartAsync(CancellationToken cancellationToken = default)
    {
        var builder = WebApplication.CreateBuilder(new WebApplicationOptions
        {
            ...
            // The web app's wwwroot (and published Blazor assets) live next to the hsh binary,
        ...
        var registry = Ctx.Get<HshRpcRegistry>("rpc");
        ...
        app.MapHub<HshRpcHub>(_config.HubPath);
        _map(app);
    ...
    /// the <c>HSH_WEB_SESSION_SECRET</c> reference (the environment or the managed store), or
    ...
    {
        const string secretRef = "HSH_WEB_SESSION_SECRET";
