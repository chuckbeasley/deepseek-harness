...
public sealed class HeadlessRun : ILoaderPlugin
{
    ...
    public ValueTask<IDisposable?> ApplyAsync(Harness.Cordis.Core.Context ctx, object? config)
    {
        ...
        {
            Console.Error.WriteLine("hsh: headless needs a task argument (hsh --profile headless \"task\")");
            Exit(ctx, 1);
            return ValueTask.FromResult<IDisposable?>(null);
        }
        var loop = ctx.Get<Harness.AgentLoop.AgentLoop>("agentLoop")
            ?? throw new InvalidOperationException("hsh: headless requires the \"agentLoop\" row");
        ...
        var ready = ctx.Get<AppReady>("appReady")
            ?? throw new InvalidOperationException("hsh: headless requires the appReady launcher fact");
        ready.OnReady(() =>
        {
            ...
            var driver = loop.GetLoop(sessionId)
                ?? throw new InvalidOperationException("hsh: headless published no loop");
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    // stderr as "hsh: reasoning:" blocks, and an errored turn appends the
                ...
                {
                    Console.Error.Write($"hsh: {error.Message}\n");
                    Exit(ctx, 1);
                }
            });
        });
    ...
    private static string StderrFromSession(Harness.Session.Session session)
    {
        ...
        return output.ToString() + $"hsh: {error.Failure.Code}: {error.Failure.Message}\n";
    }

    private static void Exit(Harness.Cordis.Core.Context ctx, int code)
    {
        var exit = ctx.Get<AppExit>("appExit")
            ?? throw new InvalidOperationException("hsh: headless requires the appExit launcher fact");
        exit.Exit(code);
    }
}
