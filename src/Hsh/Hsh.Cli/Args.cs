...
/// <summary>One resolved <c>hsh</c> invocation (port of the TS <c>HshInvocation</c> union).</summary>
public abstract record HshInvocation
{
    /// <summary>Boot a named profile and hand it the invocation's inner arguments.</summary>
    public sealed record ProfileInvocation(string Profile, IReadOnlyList<string> Patches, IReadOnlyList<string> Args) : HshInvocation;

    /// <summary>Print a composed profile tree and exit without booting.</summary>
    public sealed record DumpConfigInvocation(string Profile, bool DefaultOnly, IReadOnlyList<string> Patches) : HshInvocation;

    /// <summary>Manage a profile's plugins over its manifest.</summary>
    public sealed record PluginInvocation(string Profile, IReadOnlyList<string> Args) : HshInvocation;
}

/// <summary>Outcome of one argv parse: a resolved invocation, or an exit the parser already printed.</summary>
public sealed record ParseResult(HshInvocation? Invocation, int ExitCode)
{
    /// <summary>Successful parse; the caller runs the invocation.</summary>
    public static ParseResult Ok(HshInvocation invocation) => new(invocation, 0);
...
/// parser does not recognize starts the inner arguments, so <c>hsh --profile web -h</c> prints the
...
public static class Args
{
    ...
    private const string Description =
        "hsh: boot a DeepSeek Harness profile — an ordered stack of plugin-bundle patch layers under your own overrides.";

    private const string HelpExamples = """
        Examples:
          hsh --profile web                          boot the web profile (same as: hsh web)
          hsh --profile headless "run the tests"     answer one task, print the result, and exit
          hsh --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
          hsh --profile tui --resume <session>       arguments after the launcher flags reach the app
          hsh --profile web --help                   the web app's own flags and help
          hsh plugin --profile tui add <package>     install a plugin into the tui profile
        """;
    ...
    public static ParseResult ParseHshArgs(IReadOnlyList<string> argv, string version)
    {
    ...
    private static ParseResult ResolveRoot(LauncherOptions options)
    {
        ...
        {
            // With the app owning -h, the launcher's own help is what a bare `hsh -h` (no profile
    ...
    private static ParseResult ResolveBoot(string profile, LauncherOptions options)
    {
        ...
        {
            return ParseResult.Ok(new HshInvocation.ProfileInvocation(profile, patches.ToArray(), options.Inner.ToArray()));
        }
        if (options.DumpConfig && options.DumpDefaultConfig)
        {
        ...
        return ParseResult.Ok(new HshInvocation.DumpConfigInvocation(profile, defaultOnly, patches.ToArray()));
    }
    ...
    private static ParseResult RunPluginCommand(IReadOnlyList<string> argv, LauncherOptions parent)
    {
        ...
        return ParseResult.Ok(new HshInvocation.PluginInvocation(profile, args));
    }
    ...
    {
        Console.Out.WriteLine("Usage: hsh [options] [args...]");
        Console.Out.WriteLine();
        Console.Out.WriteLine(Description);
        Console.Out.WriteLine();
        Console.Out.WriteLine("Arguments:");
        Console.Out.WriteLine("  args  arguments for the booted profile's app (see: hsh --profile <name> --help)");
        Console.Out.WriteLine();
        Console.Out.WriteLine("Options:");
        Console.Out.WriteLine("  -V, --version         output the version number");
        Console.Out.WriteLine("  --profile <name>      the profile under $HSH_HOME/profiles to boot");
    ...
    {
        Console.Out.WriteLine("Usage: hsh plugin [options] [args...]");
        Console.Out.WriteLine();
