...
public static class SpineRegistry
{
    ...
    public static void RegisterAll(PluginCatalog catalog)
    {
        ...
        catalog.Register("sessionPersistence", new SpinePlugin("sessionPersistence", (ctx, config) =>
        {
            var root = ConfigString(config, "root")
                ?? Path.Combine(ctx.Get<string>("hshProfileDir") ?? ".", "sessions");
        ...
        catalog.Register("replay", new SpinePlugin("replay", (ctx, _) =>
        {
            ...
            // The fixture path is required (Config.file or $HSH_SNAPSHOT_FILE) and the provider
            // route follows the recorded request header ($HSH_SNAPSHOT_PROVIDER).
            var file = ConfigString(_, "file") ?? Environment.GetEnvironmentVariable(Harness.Llm.Replay.SnapshotEnv.FileEnv);
            if (string.IsNullOrEmpty(file))
            {
                throw new InvalidOperationException(
                    "replay requires a fixture path (Config.file or $HSH_SNAPSHOT_FILE)");
            }
            ...
            var handle = Harness.Llm.Replay.ReplayInstall.Install(ctx.Llm(), new Harness.Llm.Replay.ReplayConfig
            {
                ...
                Models = ParseModelMetadata(Environment.GetEnvironmentVariable("HSH_SNAPSHOT_MODEL_META")),
            });
        ...
        catalog.Register("fs", new SpinePlugin("fs", (ctx, config) =>
        {
            ...
            {
                var envDiffBasis = Environment.GetEnvironmentVariable("HSH_FS_DIFF_BASIS_MAX_BYTES");
        ...
        catalog.Register("shell", new SpinePlugin("shell", (ctx, config) =>
        {
            ...
            {
                ShellPath = ConfigString(config, "shellPath") ?? Environment.GetEnvironmentVariable("HSH_SHELL_PATH") ?? (OperatingSystem.IsWindows() ? "cmd.exe" : "sh"),
        ...
        catalog.Register("web", new SpinePlugin("web", (ctx, _) =>
        {
            ...
            if (Environment.GetEnvironmentVariable("HSH_SNAPSHOT_WEB_FETCH") == "1")
            {
            ...
            // channel is HSH_SNAPSHOT_WEB_SEARCH_BASE_URL; the recorded error fixture serves the
            // Messages endpoint on the recorded authority).
            var searchBase = Environment.GetEnvironmentVariable("HSH_SNAPSHOT_WEB_SEARCH_BASE_URL")
                ?? Environment.GetEnvironmentVariable("DEEPSEEK_SEARCH_BASE_URL");
            if (searchBase is not null)
            {
                if (Environment.GetEnvironmentVariable("HSH_SNAPSHOT_WEB_SEARCH_FIXTURE") == "1")
                {
                ...
                var apiKey = Environment.GetEnvironmentVariable("HSH_SNAPSHOT_WEB_SEARCH_API_KEY")
        ...
        catalog.Register("settings", new SpinePlugin("settings", (ctx, config) =>
        {
            ...
            var hshHome = Environment.GetEnvironmentVariable("HSH_HOME")
                ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".hsh");
            return new Harness.Settings.FileSettingsProvider(ctx, ConfigString(config, "path") ?? Path.Combine(hshHome, "settings.json"));
        }));
        ...
        {
            var root = ConfigString(config, "root")
                ?? Path.Combine(ctx.Get<string>("hshProfileDir") ?? ".", "storage");
        ...
        {
            var root = ConfigString(config, "root")
                ?? Environment.GetEnvironmentVariable("HSH_SNAPSHOT_SPILL_ROOT")
                ?? Path.Combine(ctx.Get<string>("hshProfileDir") ?? ".", "spill");
        ...
        {
            var root = ConfigString(config, "root")
                ?? Path.Combine(ctx.Get<string>("hshProfileDir") ?? ".", "attachments");
        ...
        {
            var root = Path.Combine(ctx.Get<string>("hshProfileDir") ?? ".", "presets");
        ...
        {
            var root = ConfigString(config, "root")
                ?? Path.Combine(Environment.CurrentDirectory, ".hsh", "skills");
        ...
        catalog.Register("lsp", new SpinePlugin("lsp", (ctx, _) =>
        {
            ...
            // channel is HSH_SNAPSHOT_LSP_CONFIG; shipped profiles without one register nothing).
            var configJson = Environment.GetEnvironmentVariable("HSH_SNAPSHOT_LSP_CONFIG");
        ...
        catalog.Register("sdkSubagent", new SpinePlugin("sdkSubagent", (ctx, config) =>
        {
            ...
            var hshBin = ConfigString(config, "hshBin")
                ?? throw new InvalidOperationException("sdkSubagent requires a \"hshBin\" config pointing at the SDK runtime entry");
            var provider = new Harness.Subagent.SdkOutOfProcessProvider(new Harness.Subagent.SdkOutOfProcessConfig(
                ...
                ConfigString(config, "hshHome") ?? Path.Combine(ctx.Get<string>("hshProfileDir") ?? ".", "subagent-home"),
                ConfigString(config, "cwd"),
        ...
        catalog.Register("subagentDiagnostic", new SpinePlugin("subagentDiagnostic", (ctx, _) =>
        {
            ...
            if (Environment.GetEnvironmentVariable("HSH_SNAPSHOT_SUBAGENT_DIAGNOSTIC") != "1") return null;
            var service = ctx.Get<Harness.Subagent.ISubagentService>("subagent")
        ...
        catalog.Register("subagentAcp", new SpinePlugin("subagentAcp", (ctx, _) =>
        {
            ...
            // driver is deferred).
            if (Environment.GetEnvironmentVariable("HSH_SNAPSHOT_SUBAGENT_ACP") != "1") return null;
            var service = ctx.Get<Harness.Subagent.ISubagentService>("subagent")
        ...
        catalog.Register("tui", new SpinePlugin("tui", (ctx, _) =>
        {
            ...
            var exit = ctx.Get<AppExit>("appExit")
                ?? throw new InvalidOperationException("hsh: tui requires the appExit launcher fact");
            exit.Exit(code);
            return null;
        }));
        catalog.Register("rpc", new SpinePlugin("rpc", (ctx, _) =>
        {
            var registry = new Harness.Web.Host.HshRpcRegistry(ctx);
            var sessions = ctx.Get<Harness.Session.SessionStore>("sessions")
        ...
        catalog.Register("webCore", new SpinePlugin("webCore", (ctx, _) =>
        {
            ...
            var rpc = ctx.Get<Harness.Web.Host.HshRpcRegistry>("rpc");
            if (rpc is not null)
            {
        ...
        catalog.Register("webHost", new SpinePlugin("webHost", (ctx, config) =>
        {
            ...
            var host = new Harness.Web.Host.WebHostService(
                ...
                configure: builder => builder.Services.AddHshApp(slots, pageAssemblies),
                map: app => app.MapHshApp(pageAssemblies?.List()));
            ...
            {
                Console.WriteLine($"hsh web: {host.Fence.AuthenticatedUrl(host.ListenUrl)}");
            }
            return host;
        }));
        catalog.Register("sdkRuntime", new SpinePlugin("sdkRuntime", (ctx, _) =>
        {
            ...
            transport.Start();
            var exit = ctx.Get<AppExit>("appExit")
                ?? throw new InvalidOperationException("hsh: sdkRuntime requires the appExit launcher fact");
            _ = Task.Run(async () =>
            {
        ...
        catalog.Register("acpRuntime", new SpinePlugin("acpRuntime", (ctx, config) =>
        {
            ...
            // HSH_SNAPSHOT_PROVIDER/MODEL instead.
            var key = Environment.GetEnvironmentVariable("DEEPSEEK_API_KEY");
            ...
            transport.Start();
            var exit = ctx.Get<AppExit>("appExit")
                ?? throw new InvalidOperationException("hsh: acpRuntime requires the appExit launcher fact");
            _ = Task.Run(async () =>
            {
                await transport.InputEnded;
                exit.Exit(0);
        ...
        catalog.Register("hooksClaudeCode", new SpinePlugin("hooksClaudeCode", (ctx, config) =>
        {
            var path = ConfigString(config, "configPath")
                ?? Environment.GetEnvironmentVariable("HSH_HOOKS_CC_CONFIG");
        ...
        {
            var path = ConfigString(config, "configPath")
                ?? Environment.GetEnvironmentVariable("HSH_HOOKS_CODEX_CONFIG");
            if (path is null) return null; // no hooks configured: the row is a no-op
        ...
        {
            // Provider-routed request retry (port of hsh-llm-retry): the recorded corpus used the
    ...
    private static IReadOnlyDictionary<string, Harness.Llm.LlmModelMetadata>? ParseModelMetadata(string? json)
    {
        ...
        {
            throw new InvalidOperationException($"HSH_SNAPSHOT_MODEL_META is not valid model metadata: {error.Message}");
        }
    }
    ...
    /// <summary>The corpus LSP server configuration parsed from HSH_SNAPSHOT_LSP_CONFIG.</summary>
    ...
    private static LspServerConfig ParseLspConfig(string json)
    {
        try
        {
            ...
            {
                throw new InvalidOperationException("HSH_SNAPSHOT_LSP_CONFIG: command must be a non-empty string");
            }
        ...
        {
            throw new InvalidOperationException($"HSH_SNAPSHOT_LSP_CONFIG is not valid server configuration: {error.Message}");
        }
    }
...
internal sealed class SpinePlugin : ILoaderPlugin
{
    ...
    public ValueTask<IDisposable?> ApplyAsync(Harness.Cordis.Core.Context ctx, object? config)
    {
        ...
        {
            throw new InvalidOperationException($"hsh: spine row \"{_name}\" failed to load: {error.Message}", error);
        }
