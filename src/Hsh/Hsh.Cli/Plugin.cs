...
/// <summary>
/// <c>hsh plugin --profile &lt;name&gt; &lt;args...&gt;</c> — profile plugin management: manifest
/// editing of the <c>hsh.profile.bundles</c> list in <c>profile.json</c> (<c>add
...
{
    /// <summary>
    /// Run one <c>hsh plugin</c> invocation: init if needed, then manage the bundle list.
    /// </summary>
    ...
    public static int RunPlugin(string profile, IReadOnlyList<string> args)
    {
        ...
        if (!File.Exists(Path.Combine(dir, ProfileBoot.ProfileManifestFilename)))
        {
            ...
            Console.Error.WriteLine($"hsh: initialized profile {profile} at {dir}");
        }
        ...
        switch (verb)
        {
            ...
            default:
                Console.Error.WriteLine(
                    $"hsh: unknown plugin verb \"{verb}\": the .NET port manages profile bundles directly "
    ...
    private static async Task<int> AddBundleAsync(string dir, string profile, IReadOnlyList<string> args)
    {
        if (args.Count < 2)
        {
            Console.Error.WriteLine("hsh: plugin add needs a bundle name");
            return 1;
        }
        var bundle = args[1];
        // The NuGet install path: hsh plugin add <packageId> --source <feed> resolves the
        ...
        if (sourceIndex >= 0)
        {
            ...
            {
                Console.Error.WriteLine("hsh: plugin add --source needs a feed (a flat-container URL or local directory)");
                return 1;
            }
            try
            {
            ...
            {
                Console.Error.WriteLine($"hsh: failed to install bundle \"{bundle}\": {error.Message}");
                return 1;
            }
        }
        ...
        Console.Out.WriteLine($"hsh: added {bundle} to profile {profile}");
        return 0;
    }
    ...
    private static int RemoveBundle(string dir, string profile, IReadOnlyList<string> args)
    {
        if (args.Count < 2)
        {
            Console.Error.WriteLine("hsh: plugin remove needs a bundle name");
            return 1;
        }
        var bundle = args[1];
        var manifest = ProfileBoot.ReadProfileManifest(dir);
        ...
        Console.Out.WriteLine($"hsh: removed {bundle} from profile {profile}");
        return 0;
    }
    ...
    {
        manifest.Hsh ??= new HshManifestSection();
        manifest.Hsh.Profile ??= new HshProfileManifest();
        manifest.Hsh.Profile.Bundles ??= new List<string>();
        return manifest.Hsh.Profile.Bundles;
    }
}
