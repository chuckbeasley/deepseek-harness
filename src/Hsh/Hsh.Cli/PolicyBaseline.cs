...
public static class PolicyBaseline
{
    ...
    public static string SandboxPolicyText(string mode, string workspaceRoot)
        => mode switch
        {
            "read-only" => "Current HSH file policy: read-only. Any available operation enforced by the HSH file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.",
            "workspace-write" => $"Current HSH file policy: workspace-write. Any available operation enforced by the HSH file sandbox may modify files under the session workspace: {System.Text.Json.JsonSerializer.Serialize(workspaceRoot)}. Some platform temporary areas may also be writable.",
            "danger-full-access" => "Current HSH file policy: danger-full-access. The HSH file sandbox does not restrict file modifications by available operations.",
...
public sealed class PolicyBaselinePlugin : ILoaderPlugin
{
    ...
    {
        // The permission preset comes from $HSH_PERMISSION_MODE when set (the TS headless
        ...
        var preset = Environment.GetEnvironmentVariable("HSH_PERMISSION_MODE") is { Length: > 0 } envPreset
            ? envPreset
