...
public sealed record LocalCredentialsConfig(
    /// <summary>Provider-managed credentials file path; defaults to <c>&lt;hshHome&gt;/.credentials.env</c>.</summary>
    string? ManagedPath = null,
    /// <summary>Harness home used when <see cref="ManagedPath"/> is omitted; defaults to <c>$HSH_HOME</c> or <c>~/.hsh</c>.</summary>
    string? HshHome = null,
    /// <summary>Invocation-project <c>.env</c> fallback path; defaults to <c>&lt;cwd&gt;/.env</c>.</summary>
    string? ProjectEnvPath = null,
    /// <summary>User <c>.env</c> fallback path; defaults to <c>&lt;hshHome&gt;/.env</c>.</summary>
...
/// &gt; managed &lt;hshHome&gt;/.credentials.env   (provider-managed, writable)
/// &gt; &lt;cwd&gt;/.env                    (read-only fallback)
/// &gt; &lt;hshHome&gt;/.env                   (read-only fallback)
/// </code>
...
/// <c>@deepseek-ai/hsh-credentials-local</c> (no file watcher and no cross-process writer lock in
...
public sealed class LocalCredentialsProvider : Service, ICredentialsService
{
    ...
    public const string HshHomeEnvVar = "HSH_HOME";
        ...
        : base(ctx, "credentials")
    {
        ...
        var hshHome = cfg.HshHome
            ?? Environment.GetEnvironmentVariable(HshHomeEnvVar)
            ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".hsh");
        _managedPath = cfg.ManagedPath ?? Path.Combine(hshHome, DefaultCredentialsFilename);
        _projectEnvPath = cfg.ProjectEnvPath ?? Path.Combine(Environment.CurrentDirectory, ".env");
        _userEnvPath = cfg.UserEnvPath ?? Path.Combine(hshHome, ".env");
    ...
    private void AssertUnshadowed(string reference, string verb)
    {
        if (!string.IsNullOrEmpty(_environment(reference)))
        {
            throw new InvalidOperationException(
                ...
                + " shadowed; unset it in the shell you start hsh from instead");
        }
    }
