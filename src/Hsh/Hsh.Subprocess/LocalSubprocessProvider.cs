...
/// termination is tree-scoped. The ambient environment is scrubbed of <c>HSH_*</c> facts before
...
public sealed class LocalSubprocessProvider : Service, ISubprocessService
{
    ...
    /// <summary>Scrub the ambient <c>HSH_*</c> facts, then merge the spec's explicit entries (null tombstones remove).</summary>
    private static void BuildEnvironment(ProcessStartInfo info, IReadOnlyDictionary<string, string?>? env)
    {
        foreach (System.Collections.DictionaryEntry entry in Environment.GetEnvironmentVariables())
        {
            var key = (string)entry.Key;
            if (key.StartsWith(HshEnv.Prefix, StringComparison.Ordinal))
            {
    ...
    private sealed class OutputCollector : ISubprocessOutputReader
    {
        ...
        public void Pump(StreamReader reader)
        {
            ...
            {
                _spillPath = Path.Combine(Path.GetTempPath(), "hsh-subprocess-" + Guid.NewGuid().ToString("N") + ".spill");
