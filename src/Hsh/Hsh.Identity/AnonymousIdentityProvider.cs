...
{
    /// <summary>Environment mapping consulted for <c>HSH_HOME</c>; defaults to the process environment.</summary>
    public IDictionary<string, string?>? Env { get; init; }

    /// <summary>Explicit harness-home override, which has highest precedence over <c>$HSH_HOME</c>.</summary>
...
/// <see cref="HomePaths.ResolveHshHome"/> (<c>$HSH_HOME</c> &gt; the default <c>~/.hsh</c>),
...
public sealed class AnonymousIdentityProvider : Service, IIdentityService
{
    ...
    public static AnonymousIdentityProvider Create(Context ctx, AnonymousIdentityOptions? options = null)
    {
        ...
        var home = HomePaths.ResolveHshHome(options?.Home, options?.Env);
