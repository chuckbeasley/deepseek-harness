...
/// </list>
/// Operational environment overrides feed the SAME config fields (<c>HSH_WEB_SEARCH_PROVIDER</c> /
/// <c>HSH_WEB_FETCH_PROVIDER</c>) and are not a hidden priority chain.
/// </summary>
public sealed class WebRuntime : Service, IWebService
{
        ...
        : base(ctx, "web")
    {
        ...
        _searchProviderId = cfg.SearchProvider ?? Environment.GetEnvironmentVariable("HSH_WEB_SEARCH_PROVIDER");
        _fetchProviderId = cfg.FetchProvider ?? Environment.GetEnvironmentVariable("HSH_WEB_FETCH_PROVIDER");
    }
