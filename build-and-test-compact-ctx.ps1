...
# tests\Hsh.Compaction.Tests` (and the context/session-query peers) work normally.
...
$compaction = Get-ChildItem (Join-Path $src 'Hsh\Hsh.Compaction') -Filter '*.cs' | ForEach-Object FullName
$context = Get-ChildItem (Join-Path $src 'Hsh\Hsh.Context') -Filter '*.cs' | ForEach-Object FullName
$sessionQuery = Get-ChildItem (Join-Path $src 'Hsh\Hsh.SessionQuery') -Filter '*.cs' | ForEach-Object FullName
$compactionTests = Get-ChildItem (Join-Path $root 'tests\Hsh.Compaction.Tests') -Filter '*.cs' | ForEach-Object FullName
$contextTests = Get-ChildItem (Join-Path $root 'tests\Hsh.Context.Tests') -Filter '*.cs' | ForEach-Object FullName
$sessionQueryTests = Get-ChildItem (Join-Path $root 'tests\Hsh.SessionQuery.Tests') -Filter '*.cs' | ForEach-Object FullName

# Compile in dependency order: Cordis.Core -> Hsh.Llm -> Hsh.Session -> Hsh.Tools ->
# Hsh.Compaction -> Hsh.Context -> Hsh.SessionQuery -> the three test apps.
...
$compactionDll = Join-Path $bin 'Hsh.Compaction.dll'
$contextDll = Join-Path $bin 'Hsh.Context.dll'
$sessionQueryDll = Join-Path $bin 'Hsh.SessionQuery.dll'
$compactionTestsDll = Join-Path $bin 'Hsh.Compaction.Tests.dll'
$contextTestsDll = Join-Path $bin 'Hsh.Context.Tests.dll'
$sessionQueryTestsDll = Join-Path $bin 'Hsh.SessionQuery.Tests.dll'
...
Invoke-Csc -ExtraArgs @('-target:library', "-out:$compactionDll", "-r:$coreDll", "-r:$llmDll", "-r:$sessionDll") -Sources $compaction -Label 'Hsh.Compaction'
Invoke-Csc -ExtraArgs @('-target:library', "-out:$contextDll", "-r:$coreDll", "-r:$llmDll", "-r:$sessionDll") -Sources $context -Label 'Hsh.Context'
Invoke-Csc -ExtraArgs @('-target:library', "-out:$sessionQueryDll", "-r:$coreDll", "-r:$llmDll", "-r:$sessionDll") -Sources $sessionQuery -Label 'Hsh.SessionQuery'
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$compactionTestsDll", "-r:$coreDll", "-r:$llmDll", "-r:$sessionDll", "-r:$compactionDll") -Sources $compactionTests -Label 'Hsh.Compaction.Tests'
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$contextTestsDll", "-r:$coreDll", "-r:$llmDll", "-r:$sessionDll", "-r:$contextDll") -Sources $contextTests -Label 'Hsh.Context.Tests'
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$sessionQueryTestsDll", "-r:$coreDll", "-r:$llmDll", "-r:$sessionDll", "-r:$sessionQueryDll") -Sources $sessionQueryTests -Label 'Hsh.SessionQuery.Tests'
...
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Hsh.Compaction.Tests.runtimeconfig.json') -Encoding utf8
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Hsh.Context.Tests.runtimeconfig.json') -Encoding utf8
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Hsh.SessionQuery.Tests.runtimeconfig.json') -Encoding utf8

Write-Host ''
Write-Host '== Running Hsh.Compaction.Tests =='
& dotnet $compactionTestsDll
if ($LASTEXITCODE -ne 0) { throw "Hsh.Compaction.Tests failed (exit $LASTEXITCODE)" }

Write-Host ''
Write-Host '== Running Hsh.Context.Tests =='
& dotnet $contextTestsDll
if ($LASTEXITCODE -ne 0) { throw "Hsh.Context.Tests failed (exit $LASTEXITCODE)" }

Write-Host ''
Write-Host '== Running Hsh.SessionQuery.Tests =='
& dotnet $sessionQueryTestsDll
if ($LASTEXITCODE -ne 0) { throw "Hsh.SessionQuery.Tests failed (exit $LASTEXITCODE)" }
