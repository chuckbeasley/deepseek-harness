...
# (Hsh.Sandbox + the additive ShellSandboxInfo wiring into Hsh.Shell + Hsh.Sandbox.Tests).
#
...
# tests\Hsh.Sandbox.Tests` work normally.
...
$subprocess = Get-ChildItem (Join-Path $src 'Hsh.Subprocess') -Filter '*.cs' | ForEach-Object FullName
$sandbox = Get-ChildItem (Join-Path $src 'Hsh.Sandbox') -Filter '*.cs' | ForEach-Object FullName
$shell = Get-ChildItem (Join-Path $src 'Hsh.Shell') -Filter '*.cs' | ForEach-Object FullName
$tests = Get-ChildItem (Join-Path $root 'tests\Hsh.Sandbox.Tests') -Filter '*.cs' | ForEach-Object FullName

# Hsh.Subprocess and Hsh.Shell rely on ImplicitUsings=enable but carry no GlobalUsings.cs source
...
# Compile in dependency order: Cordis.Core -> Hsh.Llm -> Hsh.Session -> Hsh.Tools -> Hsh.Subprocess
# -> Hsh.Sandbox -> Hsh.Shell -> tests. Hsh.Sandbox depends only on Cordis.Core; Hsh.Shell gains a
# dependency on Hsh.Sandbox for the ShellRunResult.Sandbox field, so the sandbox library must be
...
$subprocessDll = Join-Path $bin 'Hsh.Subprocess.dll'
$sandboxDll = Join-Path $bin 'Hsh.Sandbox.dll'
$shellDll = Join-Path $bin 'Hsh.Shell.dll'
...
Invoke-Csc -ExtraArgs @('-target:library', "-out:$subprocessDll", "-r:$core") -Sources $subprocess -Label 'Hsh.Subprocess'
Invoke-Csc -ExtraArgs @('-target:library', "-out:$sandboxDll", "-r:$core") -Sources $sandbox -Label 'Hsh.Sandbox'
Invoke-Csc -ExtraArgs @('-target:library', "-out:$shellDll", "-r:$core", "-r:$llmDll", "-r:$toolsDll", "-r:$subprocessDll", "-r:$sandboxDll") -Sources $shell -Label 'Hsh.Shell'
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$(Join-Path $bin 'Hsh.Sandbox.Tests.dll')", "-r:$core", "-r:$llmDll", "-r:$sessionDll", "-r:$toolsDll", "-r:$subprocessDll", "-r:$sandboxDll", "-r:$shellDll") -Sources $tests -Label 'Hsh.Sandbox.Tests'
...
$shellTests = Get-ChildItem (Join-Path $root 'tests\Hsh.Shell.Tests') -Filter '*.cs' | ForEach-Object FullName
$shellTests += $implicitUsings
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$(Join-Path $bin 'Hsh.Shell.Tests.dll')", "-r:$core", "-r:$llmDll", "-r:$sessionDll", "-r:$toolsDll", "-r:$subprocessDll", "-r:$sandboxDll", "-r:$shellDll") -Sources $shellTests -Label 'Hsh.Shell.Tests'
...
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Hsh.Sandbox.Tests.runtimeconfig.json') -Encoding utf8
$runtimeConfig | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $bin 'Hsh.Shell.Tests.runtimeconfig.json') -Encoding utf8

Write-Host '== Running Hsh.Sandbox.Tests =='
& dotnet (Join-Path $bin 'Hsh.Sandbox.Tests.dll')
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host '== Running Hsh.Shell.Tests =='
& dotnet (Join-Path $bin 'Hsh.Shell.Tests.dll')
exit $LASTEXITCODE