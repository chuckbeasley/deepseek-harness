...
# tests\Dsh.Skill.Tests` (and the settings twin) work normally.
...
$llm = Join-Path $bin 'Dsh.Llm.dll'
$session = Join-Path $bin 'Dsh.Session.dll'
$tools = Join-Path $bin 'Dsh.Tools.dll'
$skill = Join-Path $bin 'Dsh.Skill.dll'
$settings = Join-Path $bin 'Dsh.Settings.dll'

# Compile in dependency order: Cordis.Core -> Cosmokit -> Schemastery -> Dsh.Llm -> Dsh.Session ->
# Dsh.Tools -> Dsh.Skill -> Dsh.Settings -> both test projects.
...
Invoke-Csc -ExtraArgs @('-target:library', "-out:$llm", "-r:$core") -Sources (Get-ChildItem (Join-Path $root 'src\Dsh\Dsh.Llm') -Filter '*.cs' | ForEach-Object FullName) -Label 'Dsh.Llm'
Invoke-Csc -ExtraArgs @('-target:library', "-out:$session", "-r:$core", "-r:$llm") -Sources (Get-ChildItem (Join-Path $root 'src\Dsh\Dsh.Session') -Filter '*.cs' | ForEach-Object FullName) -Label 'Dsh.Session'
Invoke-Csc -ExtraArgs @('-target:library', "-out:$tools", "-r:$core", "-r:$llm", "-r:$session") -Sources (Get-ChildItem (Join-Path $root 'src\Dsh\Dsh.Tools') -Filter '*.cs' | ForEach-Object FullName) -Label 'Dsh.Tools'
Invoke-Csc -ExtraArgs @('-target:library', "-out:$skill", "-r:$core", "-r:$llm", "-r:$session", "-r:$tools") -Sources (Get-ChildItem (Join-Path $root 'src\Dsh\Dsh.Skill') -Filter '*.cs' | ForEach-Object FullName) -Label 'Dsh.Skill'
Invoke-Csc -ExtraArgs @('-target:library', "-out:$settings", "-r:$core", "-r:$cosmokit", "-r:$schemastery") -Sources (Get-ChildItem (Join-Path $root 'src\Dsh\Dsh.Settings') -Filter '*.cs' | ForEach-Object FullName) -Label 'Dsh.Settings'

Invoke-Csc -ExtraArgs @('-target:exe', "-out:$(Join-Path $bin 'Dsh.Skill.Tests.dll')", "-r:$core", "-r:$llm", "-r:$session", "-r:$tools", "-r:$skill") -Sources (Get-ChildItem (Join-Path $root 'tests\Dsh.Skill.Tests') -Filter '*.cs' | ForEach-Object FullName) -Label 'Dsh.Skill.Tests'
Invoke-Csc -ExtraArgs @('-target:exe', "-out:$(Join-Path $bin 'Dsh.Settings.Tests.dll')", "-r:$core", "-r:$cosmokit", "-r:$schemastery", "-r:$settings") -Sources (Get-ChildItem (Join-Path $root 'tests\Dsh.Settings.Tests') -Filter '*.cs' | ForEach-Object FullName) -Label 'Dsh.Settings.Tests'

$runtime = $pack.Name
foreach ($name in @('Dsh.Skill.Tests', 'Dsh.Settings.Tests')) {
    $runtimeConfig = @{
        runtimeOptions = @{
            tfm = 'net10.0'
...
Write-Host '== Running Dsh.Skill.Tests =='
& dotnet (Join-Path $bin 'Dsh.Skill.Tests.dll')
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host '== Running Dsh.Settings.Tests =='
& dotnet (Join-Path $bin 'Dsh.Settings.Tests.dll')
exit $LASTEXITCODE
